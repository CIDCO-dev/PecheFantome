CREATE DEFINER=`crabnet`@`%` PROCEDURE `generer_score_20x20`()
BEGIN
			UPDATE zones_20X20 a SET score_total=0 WHERE score_total IS NULL;
			...
			FROM zones_20X20 r, polygone_par_espece p
			where st_intersects(r.shape,(SELECT p.poly)) >"0") as r2
			group by r2.OGR_FID
			) as r3 where a.OGR_FID= r3.OGR_FID)  ;
            
            UPDATE zones_20X20 a SET score_frequentation=0 WHERE score_frequentation IS NULL;
			
            update zones_20X20 a
			set a.score_engins = (select r1.nb_engin from (select r.*,e.id, 
			count(*) as nb_engin
			FROM zones_20X20 r, dfo_engins e
			where ST_WITHIN(e.position, r.SHAPE) >"0"
			group by OGR_FID) as r1 where a.OGR_FID= r1.OGR_FID);

			UPDATE zones_20X20 a SET score_engins=0 WHERE score_engins IS NULL;
